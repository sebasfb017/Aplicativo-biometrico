...
from database_conn.connection import BASE_DIR, db_conn
...
def sync_all_devices(devices_list: list):
    ...
    for dev in devices_list:
        ip = dev["ip"]
        try:
            port = int(dev.get("port", DEFAULT_PORT))
        except Exception:
            port = DEFAULT_PORT
        try:
            ...
            password = 0
        try:
            timeout = int(dev.get("timeout", DEFAULT_TIMEOUT))
        except Exception:
            timeout = DEFAULT_TIMEOUT
        # timeout = max(timeout, 30)  # Removido: respetar configuración del usuario
        ...
        try:
            ...
            device_cache[ip] = {"users": users, "templates": templates, "success": True}
            ...
            logs.append(
                f"✅ {ip}: Extraídos {len(users)} usuarios y {len(templates)} huellas."
            )
        except Exception as e:
            device_cache[ip] = {"success": False}
    ...
    for dev in devices_list:
        ...
        if not users_to_add and not templates_to_add:
            logs.append(
                f"🚀 {ip}: 0 usuarios inyectados, 0 huellas inyectadas. (Ya está al día)"
            )
            continue
        ...
        try:
            ...
            for uid_str, missing_fingers in templates_to_add.items():
                ...
                time.sleep(
                    0.1
                )  # Pausa para que la flash del biométrico procese la huella
                templates_created += len(missing_fingers)

            logs.append(
                f"🚀 {ip}: {users_created} usuarios inyectados, {templates_created} huellas inyectadas."
            )
        except Exception as e:
        ...
        with db_session() as conn:
            ...
            cur.execute(
                ...
                (
                    "system",
                    "AUTO_SYNC",
                    "Sincronización automática de marcaciones completada.",
