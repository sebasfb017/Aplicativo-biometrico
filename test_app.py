...
def test_to_excel_bytes():
    summary = pd.DataFrame(
        [{"user_id": "u", "dias_tarde": 1, "minutos_tarde_total": 2}]
    )
    detail = pd.DataFrame(
        [
            {
...
def test_generate_rotating_schedule():
    ...
    df2 = app.generate_rotating_schedule(
        2026, 2, ["08:00-12:30|14:00-18:00"], grace_minutes=1
    )
...
def test_lateness_on_break_return():
        ...
        [
            {
                ...
                "full_name": "Nathalia Test",
                "profile_id": "Administrativo",
            }
        ]
    )
    app.upsert_employees_df(emp_df)
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id FROM attendance_raw WHERE user_id = 'emp_break_test' AND ts = '2026-05-25 14:03:00'"
    )
...
def test_auxiliar_privilege_mapping(monkeypatch):
    ...
    session_state["user"] = {"role": "empleado", "emp_subarea": "Talento humano"}
    ...
    error_called = False
    stop_called = False
    session_state["user"] = {"role": "empleado", "emp_subarea": "Nomina"}
    app.require_role("nomina")
    assert not error_called
    assert not stop_called
    ...
    error_called = False
    stop_called = False
    session_state["user"] = {"role": "empleado", "emp_subarea": "Calidad"}
    try:
        app.require_role("nomina")
    except Exception as e:
        assert str(e) == "stop"
...
def test_exceptions_filtering_and_kpis():
    ...
    from database_conn.queries import get_exceptions_df, upsert_exception
    ...
    df["temp_date_start"] = pd.to_datetime(df["Fecha"]).dt.date
    df["temp_date_end"] = pd.to_datetime(df["Fecha Fin"]).dt.date
    df["temp_date_end"] = df["temp_date_end"].fillna(df["temp_date_start"])

    filtered_date = df[
        (df["temp_date_end"] >= start_f) & (df["temp_date_start"] <= end_f)
    ]
...
def test_attendance_deduplication():
    ...
    data = [
        ...
        {
            "user_id": "1001",
            "ts": "2026-06-22 08:00:00",
            "punch": 0,
        },  # Duplicado exacto
        {
            "user_id": "1001",
            "ts": "2026-06-22 08:01:30",
            "punch": 0,
        },  # Doble marcación (90s después)
        {
            "user_id": "1001",
            "ts": "2026-06-22 08:05:00",
            "punch": 0,
        },  # Válido (3min 30s después)
        {
            "user_id": "1002",
            "ts": "2026-06-22 08:00:10",
            "punch": 0,
        },  # Otro usuario (válido)
    ]
