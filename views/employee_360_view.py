...
from services.analytics import fetch_attendance_between, schedule_for_user_date
...
def page_employee_360():
    ...
    if user_role not in ["admin", "nomina"]:
        ...
        st.stop()
    st.markdown(
        "<h2 style='text-align:center; font-weight:700;'>📇 Expediente 360</h2>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p style='text-align:center; color:gray; font-size:1.1rem; margin-bottom:2rem;'>Vista gerencial avanzada del historial de tiempos por empleado.</p>",
        unsafe_allow_html=True,
    )
    ...
    with col1:
        selected_year = st.number_input(
            "Año", min_value=2020, max_value=2100, value=today.year, step=1
        )
    with col2:
        selected_month = st.number_input(
            "Mes", min_value=1, max_value=12, value=today.month, step=1
        )
    with col3:
    ...
    with st.spinner("Compilando biografía de tiempos..."):
        exc_df = get_employee_exceptions(
            selected_year, selected_month, selected_user_id
        )
        exceptions_dict = {row["fecha"]: row["tipo"] for _, row in exc_df.iterrows()}

        raw_df = get_employee_raw_punches(
            selected_year, selected_month, selected_user_id
        )
        if not raw_df.empty:
        ...
        for d in range(1, dias_del_mes + 1):
            ...
            marcaciones_str = (
                " | ".join([p.strftime("%H:%M") for p in dia_punches])
                if dia_punches
                else "Sin registros"
            )

            am_punches = [
                p.strftime("%H:%M") for p in dia_punches if p.time() < time(13, 0)
            ]
            pm_punches = [
                p.strftime("%H:%M") for p in dia_punches if p.time() >= time(13, 0)
            ]
            ...
            if tipo_dia in ["retardo", "falta", "huerfano"]:
                emoji = (
                    "⏰"
                    if tipo_dia == "retardo"
                    else ("❌" if tipo_dia == "falta" else "⚠️")
                )
                desc = (
                    f"Llegó {late_min} min tarde"
                    if tipo_dia == "retardo"
                    else (
                        "No se presentó al turno"
                        if tipo_dia == "falta"
                        else "Marcó en día libre"
                    )
                )
                feed_incidentes.append(
                    {
                        ...
                        "turno": turno_str,
                        "marcado": " | ".join(
                            [p.strftime("%H:%M") for p in dia_punches]
                        )
                        if dia_punches
                        else "Sin registros",
                        "clase": tipo_dia,
...
"""
    ...
    for cell in calendar_cells:
        ...
        if cell["type"] != "libre" and cell["type"] != "empty":
            inner_content += (
                f'<div class="info-line info-turno">Turno: {cell["turno"]}</div>'
            )
        ...
        if cell["has_punches"]:
            ...
            inner_content += '<div class="info-line info-falta">Inasistencia</div>'

        if cell["novedad"] != "-":
            inner_content += (
                f'<div class="info-line info-novedad">⚠️ {cell["novedad"]}</div>'
            )

        if cell["es_festivo"]:
            inner_content += '<div class="info-line" style="background: rgba(13,202,240,0.2); color: #0dcaf0; font-weight: 600; border: 1px solid rgba(13,202,240,0.4);">🎉 Festivo</div>'
...
"""
    ...
    if not feed_incidentes:
        st.success(
            f"¡Excelente historial! **{selected_user_name}** no tuvo ninguna inasistencia o retardo durante este periodo."
        )
    else:
        feed_html = """
<style>
