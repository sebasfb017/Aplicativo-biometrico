...
def edit_attendance_dialog(record_id: int):
    ...
    new_punch_key = st.selectbox(
        "Tipo de Marcación (Punch)",
        options=punch_keys,
        format_func=lambda x: punch_opts.get(x, str(x)),
        index=punch_idx,
    )
    ...
    if submitted:
        new_ts = datetime.combine(new_date, new_time).isoformat(
        ...
        try:
            ...
            if is_manual == 1:
                ...
                """,
                    (
                        ...
                        int(new_punch_key),
                        int(rec["uid"]),
                        str(rec["downloaded_at"]),
                    ),
                )
            conn.commit()
            log_audit(
                "EDIT_ATTENDANCE",
                f"Marcación #{record_id} modif: {rec['ts']} -> {new_ts}",
            )
            ...
            st.error(
                "Error: Ya existe un registro idéntico (mismo usuario, fecha, hora y tipo)."
            )
    ...
    if submitted_del:
        try:
            ...
            conn.commit()
            log_audit(
                "DELETE_ATTENDANCE", f"Marcación #{record_id} eliminada/ignorada."
            )
...
def page_view_attendance():
    ...
    st.write(
        "Consulta y exporta la analítica en bruto de los accesos biométricos registrados."
    )
    ...
    with st.expander("🔍 Buscador y Filtros Avanzados", expanded=False):
        ...
        with col1:
            start_date = st.date_input(
                "Desde el", value=date.today() - timedelta(days=7)
            )
        with col2:
            end_date = st.date_input("Hasta el", value=date.today())
        with col3:
            selected_device = st.selectbox(
                "Reloj de Origen", options=["Todos los Dispositivos"] + devices
            )
        with col4:
        ...
        with col_f2:
            ...
            selected_subarea = st.selectbox(
                "Filtrar por Sub-área", options=subareas_opts
            )
        with col_f3:
    ...
    if df.empty:
        ...
        if df.empty:
            st.info(
                "💡 No hay llegadas tarde registradas para los filtros seleccionados."
            )
        else:
            ...
            if is_admin:
                st.info(
                    f"💡 Selecciona una marcación para editar manualmente su hora o eliminarla. Total de registros: {len(df)}"
                )
            else:
