...
from services.zk_service import (
    ...
    download_attendance_from_device,
    get_device_users_status,
    load_devices,
    save_devices,
    sync_all_devices,
    sync_device_time,
...
def edit_device_dialog(device_idx, devices_list):
    ...
    with col1:
        new_port_str = st.text_input(
            "Puerto (Default 4370)", value=str(d.get("port", 4370))
        )
    with col2:
        new_pwd_str = st.text_input("Contraseña", value=str(d.get("password", 0)))

    new_timeout = st.number_input(
        "Timeout (s)", value=int(d.get("timeout", 10)), min_value=1, step=1
    )
    ...
    with c1:
        if st.button("💾 Guardar Cambios", type="primary", use_container_width=True):
            ...
            if save_devices(devices_list):
                ...
                time.sleep(0.75)
                st.session_state["devices_table_version"] = (
                    st.session_state.get("devices_table_version", 0) + 1
                )
...
def edit_remote_user_dialog(user_data, device, devices_list):
    ...
    """
    st.write(
        f"Editando Cédula: **{user_data['user_id']}** (Encontrado en {device.get('name', device['ip'])})"
    )

    new_name = st.text_input("Nombre Corto (Ej. J. PEREZ)", value=user_data["name"])
    apply_all = st.checkbox(
        "Aplicar cambio (o eliminación) en TODOS los relojes de la red", value=True
    )
    ...
    with c1:
        if st.button(
            "💾 Guardar Nuevo Nombre", type="primary", use_container_width=True
        ):
...
def page_sync():
    ...
    tab_sync, tab_conf, tab_ctrl = st.tabs(
        [
            "⏬ Sincronizar Datos",
            "⚙️ Configurar Dispositivos",
            "🎛️ Centro de Control (Usuarios)",
        ]
    )

    with tab_sync:
        st.write(
            "Selecciona los relojes de los que deseas descargar las marcaciones recientes."
        )

        if not devices_config:
            st.error(
                "No hay dispositivos configurados. Ve a la pestaña '⚙️ Configurar Dispositivos' para crearlos."
            )
        else:
            statuses = st.session_state.get("device_statuses", {})

            with st.expander(
                "🛠️ Lista de Relojes Biométricos Disponibles", expanded=True
            ):
                col_exp_title, col_exp_refresh = st.columns([3, 1])
                with col_exp_title:
                    st.write(
                        "Marca o desmarca los dispositivos que quieras sincronizar:"
                    )
                with col_exp_refresh:
                    if st.button(
                        "🔄 Refrescar Red",
                        key="btn_refresh_sync_tab",
                        use_container_width=True,
                    ):
                        if "device_statuses" in st.session_state:
                            del st.session_state["device_statuses"]
                ...
                with col_btn1:
                    btn_sync = st.button(
                        "Descargar Marcaciones de los Relojes Seleccionados",
                        type="primary",
                        use_container_width=True,
                    )
                with col_btn2:
                    btn_set_time = st.button(
                        "Sincronizar Hora Biométricos", use_container_width=True
                    )
    ...
    with tab_conf:
        ...
        with cbtn1:
            if st.button(
                "➕ Agregar Nuevo Reloj", type="primary", use_container_width=True
            ):
                edit_device_dialog(None, devices_config)

        if not devices_config:
            st.info(
                "No hay dispositivos registrados en el sistema. Haz clic en 'Agregar Nuevo Reloj'."
            )

        else:
            df_dev = pd.DataFrame(devices_config)
            if "name" not in df_dev.columns:
                df_dev["name"] = df_dev["ip"]
            if "port" not in df_dev.columns:
                df_dev["port"] = 4370
            if "timeout" not in df_dev.columns:
                df_dev["timeout"] = 10
            if "password" not in df_dev.columns:
                df_dev["password"] = 0
            ...
            st.info(
                "💡 Selecciona un reloj en la tabla de abajo para poder editarlo o eliminarlo."
            )
    ...
    with tab_ctrl:
        st.write(
            "Audita y administra los usuarios almacenados directamente en la memoria interna de cada reloj biométrico."
        )
        ...
        if not devices_config:
            ...
            sel_dev_label = st.selectbox(
                "1. Selecciona el Reloj Biométrico a Consultar", list(dev_opts.keys())
            )
            ...
            st.write(
                "Iguala **todos los usuarios y huellas** entre todos los relojes biométricos. Si falta una huella en un reloj pero está en el otro, el sistema la copiará automáticamente."
            )

            if st.button(
                "🌟 Igualar Todos los Relojes Automáticamente",
                type="primary",
                use_container_width=True,
            ):
                with st.spinner(
            ...
            with c1:
                ...
                if st.button(
                    "📥 Descargar Usuarios de este Reloj", use_container_width=True
                ):
                    with st.spinner(f"Conectando a {selected_dev['ip']}..."):
                        ...
                        if err:
                            ...
                            st.info(
                                "No se encontraron usuarios en la memoria del reloj."
                            )
                        else:
                            st.session_state["dev_users_" + selected_dev["ip"]] = (
                                users_data
                            )
                ...
                if users_data:
                    ...
                    df_users["Huella Registrada"] = df_users["has_fingerprint"].apply(
                    ...
                    st.info(
                        "💡 Selecciona un usuario en la tabla para **Editar** su nombre o **Eliminarlo**."
                    )
                    ...
                    event_users = st.dataframe(
                        df_users[["user_id", "name", "Huella Registrada"]].rename(
                            columns={"user_id": "Cédula", "name": "Nombre en Reloj"}
                        ),
                        use_container_width=True,
                        hide_index=True,
                        on_select="rerun",
                        selection_mode="single-row",
                        key=f"tbl_users_{selected_dev['ip']}",
                    )
                    ...
                    if len(event_users.selection.rows) > 0:
                        ...
                        if (
                            str(sel_user["user_id"])
                            != st.session_state.last_edited_user
                        ):
                            st.session_state.last_edited_user = str(sel_user["user_id"])
                            edit_remote_user_dialog(
                                sel_user, selected_dev, devices_config
                            )
                    else:
                        st.session_state.last_edited_user = None
            ...
            st.markdown("---")
            with st.expander(
                "🛠️ Herramientas Avanzadas (Corrección de Datos Históricos)"
            ):
                st.write(
                    "Si una huella fue enrolada con el número de cédula equivocado, utiliza esta herramienta para trasladar todo el historial de marcaciones de la cédula incorrecta a la correcta."
                )
                ...
                with st.form("reassign_punches_form"):
                    ...
                    submit_reassign = st.form_submit_button(
                        "🔄 Trasladar Marcaciones Históricas", type="primary"
                    )
                    ...
                    if submit_reassign:
                        if not bad_id.strip() or not good_id.strip():
                            ...
                            try:
                                ...
                                if records_count == 0:
                                    st.warning(
                                        f"No se encontraron marcaciones históricas para la cédula {bad_id.strip()}."
                                    )
                                else:
                                    ...
                                    admin_user = st.session_state.get("user", {}).get(
                                    ...
                                    conn.commit()
                                    st.success(
                                        f"✅ ¡Éxito! Se han transferido {records_count} marcaciones a la cédula {good_id.strip()}."
                                    )
                                    st.info(
                                        "💡 IMPORTANTE: Recuerda borrar la huella equivocada desde la tabla superior y pedirle al empleado que ponga su huella nuevamente con su cédula correcta, para que el problema no se repita mañana."
                                    )
